"""
live_detection_dashboard.py - Transformer (Light Theme)
Author: Akhil Mudili | University of Galway
Run: python live_detection_dashboard.py -> opens http://localhost:5000
"""
...
EMBED_DIM=64;N_HEADS=4;N_LAYERS=2;FF_DIM=256;DROPOUT=0.1

with open(VOCAB_PATH,"rb") as f: vocab_data=pickle.load(f)
api_to_id=vocab_data["api_to_id"];PAD_ID=vocab_data["pad_id"]
UNK_ID=api_to_id.get("<UNK>",1);VOCAB_SIZE=vocab_data["vocab_size"]+1
...
class TransformerClassifier(nn.Module):
    def __init__(self):
        ...
        self.dropout=nn.Dropout(0.1);self.fc=nn.Linear(EMBED_DIM,1)
    def forward(self,x):
        pad_mask=(x==0);emb=self.pos_encoding(self.embedding(x))
    ...
    for cfg in configs:
        ...
        for fname in files:
            ...
            for ws in WINDOW_SIZES:
                ...
                with torch.no_grad():
                    logit=models[ws](tensor);prob=torch.sigmoid(logit).item()
...
<title>Ransomware Live Detection - Transformer</title>
<style>
...
.header-icon{width:44px;height:44px;background:#fef2f2;border:1.5px solid #dc2626;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:22px}
.header h1{font-size:20px;font-weight:600;color:#111}
.header p{font-size:13px;color:#888;margin-top:2px}
.model-badge{padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;background:#fef2f2;color:#dc2626;border:1px solid #fca5a5;margin-left:8px}
...
<div class="header">
  ...
  <div>
    <h1>Ransomware Live Detection <span class="model-badge">Transformer</span></h1>
    <p>Transformer model · Behavioral API call sequence analysis · University of Galway</p>
  </div>
...
if __name__=="__main__":
    ...
    models=load_models()
    print("Models loaded. Starting dashboard...")
